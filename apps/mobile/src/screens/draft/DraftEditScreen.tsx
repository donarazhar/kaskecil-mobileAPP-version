...
import { useDraft, useAuth, useMaster, useDraftDetail } from '@kas-kecil/api-client';
...
export default function DraftEditScreen() {
    ...
    const { data: draft, isLoading } = useDraftDetail(id);
    ...
    const filteredMataAnggaran = useMemo(() => {
        ...
        return mataAnggaran.filter((ma: any) => {
            ...
            if (formData.kategori === 'pengeluaran') {
                ...
                // Filter top-up accounts (ONLY 1.1.2)
    ...
    useEffect(() => {
        if (draft && !isInitialized) {
            setFormData({
                tanggal: new Date(draft.tanggal),
                kategori: draft.kategori as any,
                jumlah: draft.jumlah.toString(),
                keterangan: draft.keterangan || '',
                kode_matanggaran: draft.mata_anggaran?.kode_matanggaran || draft.kode_matanggaran || '',
            });

            // Handle lampiran if needed
    ...
    const handleImagePick = async () => {
        ...
        } catch (error) { // removed unused variable
    ...
    const handleCamera = async () => {
        try {
            ...
            const result = await ImagePicker.launchCameraAsync({ // Fixed deprecated method call
                allowsEditing: true,
                aspect: [4, 3],
                quality: 0.5,
            });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert('Sukses', 'Draft berhasil diperbarui', [
                { text: 'OK', onPress: () => navigation.goBack() }
            ]);
        } catch (error: any) {
            console.error(error);
            Alert.alert('Error', error?.response?.data?.message || 'Gagal menyimpan draft');
        }
    };
    ...
    return (
        <SafeAreaView className="flex-1 bg-white" edges={['top']}>
            <View className="flex-row items-center px-6 py-4 border-b border-gray-100">
                ...
                <Text className="text-xl font-bold text-gray-900">Edit Draft</Text>
            </View>
            ...
            <View className="p-6 border-t border-gray-100 bg-white">
                ...
                >
                    {isUpdating ? (
                        ...
                        <Text className="text-white font-bold text-lg">Simpan Perubahan Draft</Text>
                    )}
                </TouchableOpacity>
            </View>
