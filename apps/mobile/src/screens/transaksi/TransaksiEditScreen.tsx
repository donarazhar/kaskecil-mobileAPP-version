...
export default function TransaksiEditScreen() {
    ...
    const { data: transaksi, isLoading } = useTransaksiDetail(id);
    ...
    useEffect(() => {
        if (transaksi && !isInitialized) {
            setFormData({
                tanggal: new Date(transaksi.tanggal),
                kategori: transaksi.kategori as any,
                jumlah: transaksi.jumlah.toString(),
                keterangan: transaksi.keterangan || '',
                kode_matanggaran: transaksi.mata_anggaran?.kode_matanggaran || transaksi.kode_matanggaran || '',
            });

            if (transaksi.lampiran) {
                // Handle existing image if needed, but for now we only support uploading NEW image
                // Or we could display the existing oneUrl
            }

            setIsInitialized(true);
        }
    }, [transaksi, isInitialized]);

    // Reset Mata Anggaran when Category changes manually (skip on init)
    useEffect(() => {
        if (isInitialized) {
            // Only reset if it's a manual category change, not initial load
            // But category is rarely changed in Edit usually?
            // Let's keep it safe: if current code is invalid for new category, reset it
            const currentCode = formData.kode_matanggaran;
            // Logic to check validity could be complex, simplicity:
            // just clearer if user changes category.
        }
    }, [formData.kategori]);
                ...
                <View className="space-y-5">
                    ...
                    <View>
                        ...
                        <TextInput
                            ...
                            value={formatInputCurrency(formData.jumlah)}
                            onChangeText={(text) => setFormData({ ...formData, jumlah: text.replace(/\D/g, '') })}
                        />
                    </View>
                    ...
                    {formData.kategori === 'pengeluaran' && (
                        <View>
                            ...
                            {transaksi?.lampiran && !selectedImage && (
                                <View className="mt-2 bg-blue-50 p-3 rounded-lg">
                                    <Text className="text-xs text-blue-700">Sudah ada bukti foto tersimpan. Upload baru untuk mengganti.</Text>
                                </View>
                            )}
                        </View>
                    )}
            ...
            <View className="p-6 border-t border-gray-100 bg-white">
                <TouchableOpacity
                    className={clsx(
                        ...
                        (isUpdating || !formData.jumlah || !formData.kode_matanggaran) && "opacity-70"
                    )}
