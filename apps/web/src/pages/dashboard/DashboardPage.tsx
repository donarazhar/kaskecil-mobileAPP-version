...
const SimpleHorizontalBarChart = ({ data }: { data: { label: string; value: number; color: string; subLabel?: string }[] }) => {
                ...
                <div key={idx} className="space-y-1.5">
                    <div className="flex justify-between items-end text-sm">
                        <div className="flex flex-col">
                            <span className="text-gray-700 font-medium truncate w-32 sm:w-48" title={item.label}>{item.label}</span>
...
const MonthlySummaryTable = ({ data }: { data: { month: string; pengeluaran: number; pengisian: number }[] }) => (
    <div className="overflow-x-auto">
        <table className="w-full text-sm">
            <thead>
                <tr className="border-b border-gray-100">
                    <th className="text-left py-3 px-2 font-semibold text-gray-600">Bulan</th>
                    <th className="text-right py-3 px-2 font-semibold text-gray-600">Pengeluaran</th>
                    <th className="text-right py-3 px-2 font-semibold text-gray-600">Pengisian (Cair)</th>
                </tr>
            </thead>
            <tbody>
                {data.map((row, idx) => (
                    <tr key={idx} className="border-b border-gray-50 hover:bg-gray-50/50">
                        ...
                        <td className="py-3 px-2 text-right text-red-600 font-medium">{formatCurrency(row.pengeluaran)}</td>
                        <td className="py-3 px-2 text-right text-emerald-600 font-medium">{formatCurrency(row.pengisian)}</td>
                    </tr>
                ))}
            </tbody>
        </table>
...
export default function DashboardPage() {
    ...
    const { user } = useAuth();
    const [filters, setFilters] = useState<{ cabang_id?: number; unit_id?: number }>({});
    ...
    const criticalUnits = useMemo(() => {
        ...
        return summary.unit_summaries.filter(u => u.sisa_kas < 1000000 || u.persentase_pemakaian > 90);
    }, [summary]);
    ...
    const statsData = summary ? [
        ...
        },
        {
            ...
            change: '-' + ((summary.total_pengeluaran / (summary.plafon || 1)) * 100).toFixed(0) + '%',
            changeType: 'negative',
            icon: TrendingDown,
            iconBg: 'bg-red-500',
        ...
        },
        {
            ...
            change: ((summary.sisa_kas / (summary.plafon || 1)) * 100).toFixed(0) + '%',
            changeType: 'positive',
            icon: TrendingUp,
    ...
    const usagePercentage = summary ? ((summary.total_pengeluaran || 0) / (summary.plafon || 1)) * 100 : 0;
    ...
    const chartData = (isSuperAdmin && !filters.unit_id) ? topSpendingUnits : categoryData;
    const chartTitle = (isSuperAdmin && !filters.unit_id) ? "Top 5 Unit Boros" : "Estimasi Kategori";
        ...
        <div className="space-y-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">
                        ...
                        {isSuperAdmin && <span className="ml-1 hidden sm:inline">Anda memiliki akses penuh ke seluruh data unit.</span>}
                    </p>
                </div>
            </div>
            ...
            {isSuperAdmin && criticalUnits.length > 0 && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3 animate-fade-in">
                    ...
                    <div>
                        <h3 className="text-red-800 font-semibold text-sm">Perhatian: {criticalUnits.length} Unit dengan Saldo Kritis</h3>
                        <p className="text-red-600 text-sm mt-1">
                            Unit berikut memiliki saldo rendah (&lt; 1jt) atau penggunaan di atas 90%:
                        ...
                        >
                            <div className="flex items-start justify-between">
                                ...
                                <div className={cn(
                                    ...
                                    {/* @ts-ignore */}
                                    {stat.changeType === 'positive' && <ArrowUpRight size={14} className="text-emerald-700" />}
                                    {/* @ts-ignore */}
                                    {stat.changeType === 'negative' && <ArrowDownRight size={14} className="text-red-700" />}
                                    {stat.change}
                                </div>
                            </div>
                            <div className="mt-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="card-modern p-6">
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        <h2 className="text-lg font-semibold text-gray-900">Penggunaan Kas</h2>
                    </div>
                    <div className="flex justify-center">
                        <DonutChart
                            percentage={Math.min(usagePercentage, 100)}
                            color={usagePercentage > 80 ? '#EF4444' : usagePercentage > 50 ? '#F59E0B' : '#10B981'}
                            label="Kas Terpakai"
                        />
                    </div>
                    <div className="mt-6 space-y-2">
                        <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium text-red-600">{formatCurrency(summary?.total_pengeluaran || 0)}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                            <span className="text-gray-500">Saldo Berjalan</span>
                            <span className="font-medium text-emerald-600">{formatCurrency(summary?.sisa_kas || 0)}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="card-modern p-6">
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        <h2 className="text-lg font-semibold text-gray-900">Ringkasan Bulanan</h2>
                    </div>
                    {isSummaryLoading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <RecentPengeluaranList user={user} navigate={navigate} filters={filters} />

                {/* 2. Transaksi Pengisian Kas (History) */}
                <RecentPengisianList user={user} navigate={navigate} filters={filters} />
            </div>
        </div>
    );
}
...
function RecentPengeluaranList({ user, navigate, filters }: { user: any, navigate: any, filters: any }) {
        ...
        <div className="card-modern p-6">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h2 className="text-lg font-semibold text-gray-900">Pengeluaran Bulan Ini</h2>
                        <p className="text-sm text-gray-500">5 transaksi terakhir</p>
                    </div>
                </div>
                <Button variant="ghost" className="text-sm text-[#0053C5]" onClick={() => navigate('/transaksi')}>
                    Lihat Semua
                </Button>
            </div>
            ...
            <div className="space-y-3">
                {isLoading ? (
                    ...
                    transactions.map((trx: any) => (
                        <div key={trx.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-100 hover:border-red-200 transition-colors">
                            <div className="flex items-center gap-3">
                                <div>
                                    <p className="font-medium text-gray-900 line-clamp-1">{trx.keterangan}</p>
                                    <div className="flex items-center gap-2 text-xs text-gray-500">
                                        <span>{trx.tanggal?.split('T')[0]}</span>
                                        {trx.no_bukti && <span className="px-1.5 py-0.5 bg-gray-200 rounded">{trx.no_bukti}</span>}
                                    </div>
                    ...
                    <div className="text-center py-8 text-gray-500 text-sm">Belum ada pengeluaran bulan ini</div>
                )}
            </div>
        </div>
    );
...
function RecentPengisianList({ user, navigate, filters }: { user: any, navigate: any, filters: any }) {
        ...
        <div className="card-modern p-6">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h2 className="text-lg font-semibold text-gray-900">Riwayat Pengisian Kas</h2>
                        <p className="text-sm text-gray-500">5 transaksi terakhir</p>
                    </div>
                </div>
                <Button variant="ghost" className="text-sm text-[#0053C5]" onClick={() => navigate('/pengisian')}>
                    Lihat Semua
                </Button>
            </div>
            ...
            <div className="space-y-3">
                {isLoading ? (
                    ...
                    transactions.map((trx: any) => (
                        <div key={trx.id} className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-100 hover:border-emerald-200 transition-colors">
                            <div className="flex items-center gap-3">
                                <div>
                                    <p className="font-medium text-gray-900 line-clamp-1">{trx.keterangan}</p>
                                    <div className="flex items-center gap-2 text-xs text-gray-500">
                    ...
                    <div className="text-center py-8 text-gray-500 text-sm">Belum ada riwayat pengisian</div>
                )}
            </div>
        </div>
    );
