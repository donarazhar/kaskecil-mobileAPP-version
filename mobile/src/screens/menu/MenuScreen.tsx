...
function MenuItem({
  ...
  return (
    ...
    >
      <View
        className={`w-11 h-11 rounded-2xl items-center justify-center mr-4 ${danger ? "bg-red-100" : "bg-gray-100"}`}
      >
        <Icon size={20} color={danger ? "#EF4444" : "#4B5563"} />
      </View>
      <View className="flex-1">
        <Text
          className={`text-base font-semibold ${danger ? "text-red-500" : "text-gray-800"}`}
        >
          {label}
        </Text>
        {description && (
...
export default function MenuScreen() {
  ...
  const handleLogout = () => {
    Alert.alert(t("menu.logoutTitle"), t("menu.logoutConfirm"), [
  ...
  const isAdminUnit = user?.role?.name === "admin_unit";
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50" edges={["top"]}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        {/* Admin Unit Section - Hanya untuk admin_unit */}
        {isAdminUnit && (
          <MenuSection title="Master Inputan">
            <MenuItem
              icon={CreditCard}
              label="Input Akun AAS"
              description="Kelola master akun akuntansi"
              onPress={() => navigation.navigate("AkunAAS")}
            />
            <View className="h-px bg-gray-100 mx-4" />
            <MenuItem
              icon={Layers}
              ...
              onPress={() => navigation.navigate("MataAnggaran")}
            />
          </MenuSection>
        )}
        ...
        {isAdmin && (
          <MenuSection title={t("menu.adminPanel")}>
            ...
            <MenuItem
              ...
              onPress={() =>
                Alert.alert(t("common.comingSoon"), t("menu.manageUsersDesc"))
              }
            />
            <View className="h-px bg-gray-100 mx-4" />
            <MenuItem
              ...
              onPress={() =>
                Alert.alert(t("common.comingSoon"), t("menu.masterBranchDesc"))
              }
            />
          </MenuSection>
        )}
