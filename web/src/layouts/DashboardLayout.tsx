...
const mockNotifications = [
  {
    ...
    message: "3 pengajuan menunggu persetujuan",
    time: "5 menit lalu",
    unread: true,
  },
  {
    ...
    message: "Transaksi #TRX-001 telah disetujui",
    time: "1 jam lalu",
    unread: true,
  },
  {
    id: 3,
    title: "Saldo Rendah",
    message: "Saldo kas kecil Unit IT hampir habis",
    time: "2 jam lalu",
    unread: false,
  },
];
...
export default function DashboardLayout() {
  ...
  // Role checks (moved before useEffect)
  const isSuperAdmin = user?.role?.name === "super_admin";

  // Get menu items based on role (moved before useEffect)
  const masterInputanItems = isSuperAdmin
    ? masterInputanSuperAdmin
    : masterInputanAdminUnit;
  const laporanItems = isSuperAdmin
    ? laporanItemsSuperAdmin
    : laporanItemsAdminUnit;

  // Auto-expand menus based on current path (MUST be before any returns)
  useEffect(() => {
    if (!isCollapsed) {
      if (
        masterInputanItems.some((item) =>
      ...
      if (
        transaksiItemsAdminUnit.some((item) =>
          location.pathname.startsWith(item.href),
        )
      ) {
      ...
      if (
        laporanItems.some((item) => location.pathname.startsWith(item.href))
      ) {
        setMenuOpen((prev) => ({ ...prev, laporan: true }));
      }
      if (
        pengaturanItems.some((item) =>
          location.pathname.startsWith(item.href),
        ) ||
  ...
  }, [location.pathname, laporanItems, masterInputanItems, isCollapsed]);

  // Auth checking is now handled by ProtectedRoute in App.tsx
  ...
  const isMenuActive = (items: SubMenuItem[]) =>
  ...
  const NavItem = ({
    ...
    const content = (
      ...
      >
        ...
        {(!isCollapsed || mobile) && (
          <span className="font-medium animate-in fade-in zoom-in-95 duration-200">
            {label}
          </span>
        )}
      </NavLink>
  ...
  const SidebarContent = ({ mobile = false }: { mobile?: boolean }) => (
    <div className="flex flex-col h-full">
      ...
      <div
        className={cn(
          "p-6 pb-8 transition-all duration-300",
          isCollapsed && !mobile ? "px-2 items-center" : "",
        )}
      >
        <div className="flex items-center gap-3">
          ...
          {(!isCollapsed || mobile) && (
            <div className="overflow-hidden whitespace-nowrap">
              <h1 className="text-xl font-bold text-white tracking-tight animate-in fade-in slide-in-from-left-4 duration-300">
                Kas Kecil
              </h1>
              <p className="text-xs text-white/50 font-medium animate-in fade-in slide-in-from-left-4 duration-300 delay-75">
                Imprest Methode App
              </p>
            </div>
          )}
      ...
      <nav
        className={cn(
          "flex-1 px-4 space-y-1.5 overflow-y-auto overflow-x-hidden",
      ...
      >
        ...
        <div
          className={cn(
            "my-4 h-px bg-white/10",
            isCollapsed && !mobile ? "mx-2" : "mx-4",
          )}
        />
      ...
      <div className={cn("p-4 mt-auto", isCollapsed && !mobile && "px-2")}>
        <div
          className={cn(
            "h-px bg-white/10 my-3",
            isCollapsed && !mobile && "mx-1",
          )}
        />
        ...
        >
          {isCollapsed && !mobile ? (
            <Tooltip>
              <TooltipTrigger asChild>
                <Avatar
                  className="h-8 w-8 border-2 border-white/20 cursor-pointer"
                  onClick={() => logout()}
                >
              ...
              <TooltipContent>{user?.nama} (Klik untuk keluar)</TooltipContent>
            </Tooltip>
          ) : (
            <>
              ...
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="relative hover:bg-gray-100 ml-1"
                    >
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      className="hidden sm:flex items-center gap-2 pl-2 pr-3 py-1.5 h-auto hover:bg-gray-100 ml-2"
                    >
