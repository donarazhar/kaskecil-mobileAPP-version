...
import { useTransaksi, useTransaksiDetail, useMaster } from '@kas-kecil/api-client';
...
export default function TransaksiEditPage() {
    ...
    const { update, isUpdating } = useTransaksi();
    const { data: transaksi, isLoading: isLoadingDetail } = useTransaksiDetail(transaksiId);
    const { user } = useAuth();
    ...
    useEffect(() => {
        if (transaksi) {
            setFormData({
                ...
                jumlah: transaksi.jumlah?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.') || '',
                kode_matanggaran: transaksi.kode_matanggaran || '',
            });

            // Set existing attachments
            const existingFiles: typeof files = {};
            if (transaksi.lampiran) {
                existingFiles.lampiran = { url: transaksi.lampiran, isExisting: true };
            }
            if (transaksi.lampiran2) {
                existingFiles.lampiran2 = { url: transaksi.lampiran2, isExisting: true };
            }
            if (transaksi.lampiran3) {
                existingFiles.lampiran3 = { url: transaksi.lampiran3, isExisting: true };
            }
    ...
    const handleFileChange = (field: 'lampiran' | 'lampiran2' | 'lampiran3', e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            if (files[field]?.url && !files[field]?.isExisting) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const payload: any = {
                ...
                kode_matanggaran: formData.kode_matanggaran || null,
            };

            // Add new files only
            if (files.lampiran?.file) payload.lampiran = files.lampiran.file;
            if (files.lampiran2?.file) payload.lampiran2 = files.lampiran2.file;
            if (files.lampiran3?.file) payload.lampiran3 = files.lampiran3.file;

            await update({ id: transaksiId, data: payload });
            navigate('/transaksi');
        } catch (error: any) {
    ...
    useEffect(() => {
        return () => {
            Object.values(files).forEach(f => {
                if (f?.url && !f.isExisting) URL.revokeObjectURL(f.url);
            });
        };
    }, []);
    ...
    if (!transaksi) {
        return (
            <div className="max-w-2xl mx-auto space-y-6">
                <Button variant="ghost" onClick={() => navigate('/transaksi')}>
        ...
        <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div>
                ...
                <p className="text-gray-500 mt-1">
                    Perbarui data transaksi pengeluaran
                </p>
            </div>
                ...
                <CardHeader className="border-b border-gray-100 bg-gradient-to-r from-[#0053C5]/5 to-transparent">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-500">Perbarui informasi transaksi</p>
                        </div>
                    </div>
                </CardHeader>
                                ...
                                <div className="space-y-2">
                                    {files.lampiran ? (
                                        <div className="relative rounded-lg overflow-hidden border border-gray-200">
                                            ...
                                            {files.lampiran.isExisting && (
                                                <span className="absolute top-2 left-2 px-2 py-0.5 rounded bg-blue-500 text-white text-xs">
                                                    Existing
                                                </span>
                                            )}
                                        </div>
